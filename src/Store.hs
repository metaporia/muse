...
fromDB' :: DB -> Results
fromDB' DB { chrono, ..} = Results . go $ IxSet.toAscList
  (Proxy :: Proxy UTCTime)
  chrono
 where
  go = foldr f []
  f (TsIdxTup tsIdx) xs
    = let t = ts tsIdx
      in
        case snd . val $ tsIdx of
          Pgs pg -> fromEntry t (PN pg) : xs
          Dmp    -> case getOne $ dumped @= t of
            Just dumps -> fromDumps dumps
            Nothing    -> xs
          Qts -> case getOne $ quotes @= t of
            Just i ->
              (fromEntry t $ Quotation (T.unpack . getFst . val $ tsTag i)
                                       (T.unpack . getSnd . val $ tsTag i)
                                       (getThrd . val $ tsTag i)
                )
                : xs
            Nothing -> xs
          Rds -> case getOne $ reads @= t of
            Just (TsIdxTup i) ->
              ( fromEntry t
...
addDay day utc le =
 ...
 where
  ...
  tagAndUpdate (x : xs)
    ...
    = addLogEntry day x Nothing
      >>= (\t -> tagRest (AttrTag <$> t) xs)
      >>= tagAndUpdate
    | -- tag and update rest of indented; recurse
      otherwise
...
filterDumps (Search s e _ BucketList { dumpsPreds }) ixSet = case dumpsPreds of
  [] -> DumpR <$> dumped'
...
filterDialogues db (Search s e authPreds BucketList { dialoguesPreds }) ds
  | not $ null authPreds = []
  | -- when there are attr preds, skip these untagged entries
    otherwise = case dialoguesPreds of
...
filterPhrases db (Search s e authPreds BucketList { phrasesPreds }) phrases
  | null authPreds = case phrasesPreds of
    ...
    (hs, ms) -> filtermap
      ...
      phrases'
  | otherwise = case phrasesPreds of
    ([], []) -> filtermap
...
filterComments db (Search s e authPreds BucketList { commentsPreds }) cmts
  | null authPreds = case commentsPreds of
    [] -> fmap
  ...
  | otherwise = filtermap
    (\tag ->
      let c = val $ tsTag tag
      in  if satisfiesAll commentsPreds c && readSatisfies s e db tag authPreds
...
filterQuotes
  :: DB -> Search -> IxSet (TsIdxTag (Body, Attr, Maybe PgNum)) -> [Result]
filterQuotes db (Search s e authPreds BucketList { quotesPreds }) quotes
  | null authPreds = case quotesPreds of
    [] -> fmap
      (\t ->
    ...
    _ -> filtermap
      (\t ->
        ...
        in  if satisfiesAll quotesPreds (b, attr, mPg)
              then Just $ TsR (ts $ tsTag t)
                              (Quotation (T.unpack b) (T.unpack attr) mPg)
              else Nothing
      )
      quotes'
  |
 ...
 where
  ...
  readSatisfies' tag = case attr tag of
...
insertDump day content = do
  db@DB {..} <- get
  let dumps = case getOne $ IxSet.getEQ day dumped of
...
updateQuote ts b a mp tag = do
  db@DB {..} <- get
  put DB
    { quotes = updateIxSetTsIdxTag (mkTsIdxTag ts (b, a, mp) tag) quotes
    , ..
    }
  return (b, a, mp)
...
lastRead = do
  db@DB {..} <- ask
  return . fmap (val . tsIdx) . head' $ IxSet.toDescList
    (Proxy :: Proxy UTCTime)
    reads
...
view :: IO ()
view = do
  r <- bracket (openLocalStateFrom "state/DB" initDB)
               createCheckpointAndClose
